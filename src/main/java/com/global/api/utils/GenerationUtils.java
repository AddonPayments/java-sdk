...
public class GenerationUtils {
    ...
    /**
     * El ID de pedido para una solicitud inicial debe ser unico por transaccion. Este metodo genera un ID unico
     * El Order ID usa la clase de Java UUID y luego se convierte a base64. Concatenamos con la fecha de la operacion
     * el order Id generado de forma automatica. El order ID se utiliza en cada transaccion (operacion, devolucion,
     * credito...). Para acortar los caracteres maximos hemos utilizado el metodo "substring()" y hemos indicado
     * que la longitud maxima no puede superar los 36 caracteres (el servidor acepta hasta 40).
     *
     * El Order ID utiliza el UUID de Java (identificador unico universal), por lo que en teoria puede no ser posible
     * que no sea unico, pero las probabilidades de que esto ocurra son extremadamente remotas. (Wikipedia)
     ...
     *
     * @return orderId como un String
     **/
    
	static public String tiempoOrderId() {
        return tiempoOrderId(null);
    }
	
    static public String tiempoOrderId(String timestamp) {
        if(timestamp != null)
            return timestamp;
        return new SimpleDateFormat("HH_mm-dd-MM-yyyy").format(new Date());
    }
